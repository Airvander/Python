...
def calculate_card_value(card_value, time_of_entry):
    ...
    if (card_value - erp_rate) >= 0:  #Check if there is enough balance to pay ERP charge
        card_value =  card_value - erp_rate #Modify to calculate new value in cash card
        successful_deduction = True
        print('new value in cash card:',"{:.2f},".format(card_value),successful_deduction) #Modify to display new value in cash card   
    else:
        successful_deduction = False
        print('insufficent amount in cash card' ,"{:.2f},".format(card_value),successful_deduction) #Modify to display insufficent balance in cash card
